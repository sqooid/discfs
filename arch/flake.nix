  ...
  outputs = { self, nixpkgs, crane, flake-utils, rust-overlay, ... }:
    flake-utils.lib.eachDefaultSystem (localSystem:
      let
        ...
        # discfs = pkgs.callPackage ./myCrate.nix { };
        discfs = pkgs.callPackage crateExpression { };
      in
      {
        checks = {
          inherit discfs;
        ...
        packages.default = discfs;

        apps.default = flake-utils.lib.mkApp {
          drv = pkgs.writeScriptBin "my-app" ''
            ${pkgs.pkgsBuildBuild.qemu}/bin/qemu-aarch64 ${discfs}/bin/cross-rust-overlay
          '';
        };
      });
}
